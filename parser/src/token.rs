...
pub trait BigIntHelper {
  fn to_i64(&self) -> i64;
}

impl BigIntHelper for BigInt {
  fn to_i64(&self) -> i64 {
    let digits: (Sign, Vec<u32>) = self.to_u32_digits();
    println!("to_i64 digits: {:?}", digits.1);
    let mut result: u64 = 0;
    if digits.1.len() > 1 {
      result = result | digits.1[1] as u64;
      result = result << 32;
    }
    result = result | digits.1[0] as u64;
    result as i64
  }
}
